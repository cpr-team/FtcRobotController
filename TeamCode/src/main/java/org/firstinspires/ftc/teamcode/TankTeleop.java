 ...
 */
@TeleOp
public class TankTeleop extends PracticeAFL {
...
//    }
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            float max = Math.max(Math.abs(left_y)+Math.abs(right_x),1.0f);

           float fr_drive = ( right_x)/max;
           float fl_drive = ( right_x)/max;
           float br_drive = ( right_x)/max;
           float bl_drive = ( right_x)/max;

            float fr_drive_L = (left_y  )/max;
            float fl_drive_L = (left_y )/max;
            float br_drive_L = (left_y )/max;
            float bl_drive_L = (left_y  )/max;
            drive(fr_drive_L, -fl_drive_L, -br_drive_L, bl_drive_L);
